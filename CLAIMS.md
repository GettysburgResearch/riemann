...
| `L-16005` | Hard-window artifact dominance: exact error relation, rigorous $\varepsilon(T)$ bound, $O(1/j)$ decay, certification boundary $\alpha\lesssim0.4$ | `PROPOSED` | bounds `PROVED`; tables `EMPIRICAL` | `L-16001`, `O-16001` |
...
| `O-16007` | The finite Weil form's representing measure in closed form: implied residue `a(gamma) = (log c / pi^2) sin^2(gamma log c / 2)`, `N`-independent to 9-10 digits, ratio 1.000000 across 14 cutoffs while `a_1` swings 685x. **Some cutoffs are blind to a zero**, and `c = 500` (used in `O-16004`) is one of them for `gamma_1` | `PROPOSED` — measured, not derived | high-precision float, dps 150-260 | `L-16006`, `O-16004` |
| `O-16008` | Detectability: sensitivity and readability **trade against each other**. At `N=14` the criterion breaks at `|Re rho - 1/2| ~ 1e-17` but float64 sees nothing below `3e-3` -- a 14-order blind band that WIDENS with `N`. Needs `~4.5N` digits. Also **corrects the scope of `L-16004`(ii)** | `PROPOSED` — synthetic caricature | high-precision float, dps doubled on 2 rows; constants NOT converged in `M` | `L-16004`, `O-16004` |
