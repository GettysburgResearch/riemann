...
| X-0020 | Pick detector at the two-architecture convergence neighborhood (T~4.7e12): validated to `delta=1e-9`, real matrix PD | EMPIRICAL (Arb-RS trust class) | `experiments/X-0020-pick-at-frontier/` |
| X-0019 | Complete rational-square positive-anchor ladder at the atomized-minimum ordinate: null at all six anchors | EMPIRICAL (exact rational + Arb-RS) | `experiments/X-0019-anchor-ladder/` |
| X-0018 | PR #128 w=4 one-point handoff executed: b0 inside the gate to 20 digits of the reconnaissance; no counterexample | EMPIRICAL (exact rational + Arb-RS) | `experiments/X-0018-w4-anchor-run/` |
| X-0017 | Independent reimplementation of the L-0002 counting layer (winding2.py); 8/8 boxes agree incl. adversarial edges | PROVED (rule) / EMPIRICAL (agreement); closes the counting half of Q-0003 | `experiments/X-0017-winding-crosscheck/` |
| X-0016c | Primes-only detector at `Re a ~ 2`: strong point-side detection but witness magnitudes unreachable by any feasible sieve; B-splines win the prime side | EMPIRICAL | `.../X-0016-three-access/farprobe.py` |
| X-0016 | One positivity number, three disjoint computations (points / primes / zeros): ratios 1.00000000 / 1.00000899; **prime side certified** at `Re a = 2.05` (X-0016b, certified overlap) | EMPIRICAL + PROVED (the Re-a~2 prime-side certificate) | `experiments/X-0016-three-access/` |
...
| X-0015 | Tuned scalar detector: three explicit designs fail (R-0011); LDL-extracted witness works, certified end to end | EMPIRICAL | `experiments/X-0015-tuned-detector/` |
| R-0011 | Explicit filter design loses to the implicit LDL optimum by ~1e12; extraction, not replication | REFUTED (postmortem) | `NEGATIVE_RESULTS.md` |
...
| X-0007b | Completeness closures at `T=2000` AND `T=5000`: every zero simple, `\|Re rho-1/2\| <= 9.49e-77` by pinning Newton discs to scan zeros (claude-01's disjointness check was too strict) | PROVED (composition of certificates) | `experiments/X-0007-newton-isolation/completeness.py` |
...
| X-0007 | Newton sweep: all 4520 discs to `t=5000` certified simple with `\|Re rho-1/2\| <= 9.5e-77` (completeness: X-0007b) | EMPIRICAL (certified) | `experiments/X-0007-newton-isolation/` |
