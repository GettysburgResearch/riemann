...
so at this single endpoint

\[
\frac1{50}<\frac{F(184)}{M(184)}<\frac1{20}.
\]

No universal replacement interval is inferred from one endpoint or from finite scans.

### 3.3 Why a corrected constant would not close the route

Suppose, only formally, that a universal finite-packet interval

\[
\frac1{50}\le\frac FM\le\frac1{20}
\]

were proved and that the recursively observed current really belonged to the same cone. With recursive mass `rho=1/8`, the proposed current upper bound would be

\[
\frac1{20}\frac{1+\rho}{1-\rho}=rac9{140},
\]

and the formal root margin would be

\[
\frac1{50}(1-\rho)-\frac9{140}\rho
=rac{53}{5600}>0.
\]

This exact rational calculation shows that the numerical architecture is not inherently impossible. The source current, however, is not a finite `P_61` packet, so these constants do not apply.
...
be the rough primes. Let `S` swap the two positive parity channels.

### 4.1 Base packet

`B_x` is the finite `P_61` packet of Section 1. Its signed scalar is `F(x)` and its unsigned mass is `M(x)`.

### 4.2 Full packets

For `j>=1`, define `N_j(x)` as the positive direct sum of all atoms with:

- one squarefree small-prime color `d|P_61`;
- an ordered squarefree rough history `p_{i_1}<...<p_{i_r}`, all indices at least `j`;
- one base index `m>=2`;
- activation `d p_{i_1}...p_{i_r}m<=x`;
- coefficient `q_star(m)H_x(d p_{i_1}...p_{i_r}m)/sqrt(d p_{i_1}...p_{i_r}m)`;
- parity `mu(d)(-1)^r`.

For fixed `x`, every atom has product at least `2*67^r`, so only finitely many histories occur.

### 4.3 Exact first-owner identity

Every atom in `N_j(x)` has either no rough prime from the tail, in which case it belongs to `B_x`, or a unique least rough prime `p_k` with `k>=j`. Removing that prime gives an atom of `N_{k+1}(x/p_k)`, and the scale covariance preserves coefficient magnitude:
...
This is a literal disjoint direct sum. It proves first-owner uniqueness, disjoint ownership, coefficient preservation, cumulative parity, and exact source normalization.

### 4.4 Scalar and mass recursions

Let `R_j(x)` be the signed scalar observation and `U_j(x)` the unsigned mass. Then
...
The base case is `x<2p_j`, when the sum is empty and `N_j(x)=B_x`. Strict rank descent is by the finite number of admissible rough primes below `x/2`, or equivalently by the increasing owner index together with scale division. No limiting interchange is used.

The native scalar is

\[
A_X=R_1(X).
\]

### 4.5 Direct coefficient check

Writing every squarefree `k` uniquely as a small color times an ordered rough history gives the coefficient `mu(k)`. Convolving with the positive base dictionary gives

\[
R_1(X)=
\sum_{n\le X}
\frac{a_\star(n)}{\sqrt n}H_X(n),
\]

with the exact `a_star` of Section 1. Thus the source recursion and the Riesz formula are identical, not merely asymptotic.

---

## 5. The source-type no-go theorem for the frozen contraction

PR #565's `L-97102` uses a packet `P_x` in the causal identity and applies the finite ratio `F/M` to its current terms. There are only two interpretations.

### 5.1 Finite interpretation

If `P_x=B_x`, then its current

\[
B_x-p^{-1/2}SB_{x/p}
\]

is a positive finite packet and the algebraic bias calculation is typed. But a tree built from `B_x` contains no rough histories, and its root signed scalar is `F(x)`, not `R_1(x)=A_x`. It cannot prove the native scalar sign.

### 5.2 Full interpretation

If `P_x=N_j(x)`, then the root scalar is native. But

\[
N_j(x)-p_j^{-1/2}SN_{j+1}(x/p_j)
\]

still contains all histories whose first rough prime is larger than `p_j`, in addition to the base packet. Its signed scalar and unsigned mass are not `F(x)` and `M(x)`. The finite bias certificate cannot be applied to this current.

### 5.3 Low-child recombination

For `alpha=lambda p^{-1/2}`, the formal cancellation
...
is exact. If `P_x` is full, it returns the same full packet at the same scale. It is not a terminal packet, does not reduce rank, and still contains the odd rough histories. If `P_x` is finite, it remains non-native.

### 5.4 Rough-child mass

The raw coefficient inequality

\[
\sum_i\alpha_i<\frac18
\]

is true. To become a normalized source-mass estimate, it additionally needs

\[
m(U_iP_{x/p_i})\le m(P_x)
\]

for the same full packet and mass used in the recursion. Even such a normalized inequality would not supply the missing full-current scalar cone. PR #575 identifies precisely this distinction. Even when the normalized ratio remains below `1/8`, the required current cone is still absent.
...
The witness is not erased; the same full node returns. Any construction that then applies the finite `P_61` bias to `lambda N_923` has changed the packet type. The corrected recursion retains the odd contribution until the complete alternating sum is formed.
