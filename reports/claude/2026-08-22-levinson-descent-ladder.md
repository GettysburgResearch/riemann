...
level for ξ-like rigidity classes (Hellerstein–Williamson; Bergweiler–
...
- **L-105062 (PROVED)**: every rung Ξ_k keeps its zeros in the strip
  ...
  ξ^(k) are distinct points on the critical line** — the imported baseline
  rides the ladder to every rung.
...
budget; near-region monotonicity certificates ≥ 39.8×; fail-closed replay
...
matches an independent Turing-method recount; per-zero agreement with the
...
programs share no mathematical inputs — a deliberate portfolio split. Nothing
