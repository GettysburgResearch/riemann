...
def main() -> None:
    ...
    numerator_pr = numerator_symmetric[0].subs(
        {elementary_num[0][0]: P, elementary_num[1][0]: R}
    )
    denominator_pr = denominator_symmetric[0].subs(
        {elementary_den[0][0]: P, elementary_den[1][0]: R}
    )

    if (numerator_pr.free_symbols | denominator_pr.free_symbols) - {P, R, U, H}:
        raise AssertionError("unresolved formal symmetric generators")
    ...
    # The source's second assertion also needs repair: P^2 >= 4R.
    V = sp.Symbol("V", nonnegative=True)
    numerator_cone = sp.expand(sum(coefficient * P**(degree % 2) * (V+4*R)**(degree//2)
        for (degree,), coefficient in sp.Poly(numerator_pr, P).terms()))
    cone_poly = sp.Poly(numerator_cone, P, R, U, H, V)
    if not all(c >= 0 for c in cone_poly.coeffs()):
        raise AssertionError("numerator not positive on physical invariant cone")
    if sp.expand(numerator_cone.subs(V,P**2-4*R)-numerator_pr) != 0:
        raise AssertionError("cone rewrite changed the polynomial")
    if not denominator_ok:
        raise AssertionError("denominator needs separate sign proof")
    negative_terms = [(list(m),str(c)) for m,c in sp.Poly(numerator_pr,P,R,U,H).terms() if c<0]
    certificate = {"sympy_version":sp.__version__, "original_blob":"e708af8c5235b71e7382d43220236536fc3f20a8",
        "formal_substitution_repaired":True, "physical_relation":"V=P^2-4R=(A-B)^2>=0",
        "negative_original_terms":negative_terms, "original_numerator_terms":numerator_terms,
        "cone_numerator_terms":len(cone_poly.terms()), "cone_coefficients_nonnegative":True,
        "cone_numerator":[[list(m),str(c)] for m,c in cone_poly.terms()],
        "denominator_factorization":str(sp.factor(denominator_pr)),
        "strict_positive_monomial":"32*H^11*P", "rh_proved":False}
    Path(__file__).with_suffix(".certificate.json").write_text(json.dumps(certificate,indent=2)+"\n")
    ...
    result = {
        "verdict": "PASS_A_REPAIRED_RANK_TWO_CAUCHY_TRACE",
        "arithmetic_class": "EXACT_SYMBOLIC_RATIONAL",
        "packet_rank": 2,
        "numerator_coefficientwise_nonnegative": numerator_ok,
        "physical_cone_numerator_nonnegative": True,
