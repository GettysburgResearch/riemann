...
SLO = (83, 100)               # rectangle
SHI = (99, 100)
TLO = (1221, 100)
THI = (1238, 100)
RENL = (1, 10)                # Cauchy enlargement r
X_CUT = 18                    # lattice cutoff on Q (raised from 14:
                              # the winding sum accumulates per-sample
                              # angle widths over ~4000 steps, so the
                              # tail box must be ~1e-13, not ~3e-11)
...
S_ABS_HI = 13.0    # |s| and |1-s| stay below this on our rectangle
...
def zeta_abs_bound(splo, sphi, tpabs_lo, tpabs_hi):
    """L2 bound for |zeta(s')|, sigma' in [splo, sphi] > 0,
    |Im s'| in [tpabs_lo, tpabs_hi]."""
    N = 40
    S = iv.mpf(0)
    ...
    S = S + sabs * iv.exp(-Alo * iv.log(iv.mpf(N))) / Alo
    return mp.mpf(S.b)
...
def main():
    ...
    out = {
        ...
        "float_zero_location": "0.9083696189931 + 12.2962410860869 i",
    ...
    json.dump(out, open('epstein/wall_complex.json', 'w'), indent=1)
