...
- Departure-time bisection: the on/off classification at the bisection endpoints
  resolves line pairs down to separation ~2e-4; closer than that, tau* carries that
  (documented) classification resolution — the transition is continuous, so this is
  inherent, not a shortcut.
- COORDINATOR BUDGET REDUCTIONS (mid-run guidance from the coordinating session,
  ...
  * E4 reduced from 8 directions to 4 (0, 45, 315 computed; 180 derived by the EXACT
    ...
    lab.selftest); t-window reduced to (0, {E4_T1}); radius bisection to bracket 0.005
    (2 decimals). March capped at r_max = {E4_RMAX} (directions with no off-line pair
...
- O1 direct sum: radius 2000 (N = 4e6) with an exact-(A(N)+1) tail estimate,
  triangular(Cesaro)-averaged over 257 truncation points; the achieved agreement is
  recorded in oracles.json (the circle-problem residual limits a single truncation to
  ~11 digits; the origin-term correction and the averaging are what buy 15+).
...
- No reduction was applied to the oracle thresholds, the E2 atlas, the E3 t-window,
  or working precision (dps=40 everywhere zeros are located).
"""
