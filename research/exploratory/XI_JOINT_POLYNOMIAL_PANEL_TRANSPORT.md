# Fixed-panel joint-polynomial Xi transport

Status: PREREGISTERED BEFORE NEW PANEL ARITHMETIC.
Authoring base: JP96543552b5bc5976d96c35123bfb82e609a27972.
This is a separate successor; all JP/QT science and prior reviews remain
unchanged.

## Fixed design and selection boundary

Evaluate exactly the26 inherited QT real critical points in frozen order.
Index19 is a KNOWN POSITIVE CONTROL from JP and is excluded from the
primary25-untried-point outcome. The other25 points have not been evaluated
under this joint-polynomial criterion by this author. They are existing
QT/HA source points, not new source windows or a blind zero census.

For EVERY point fix the same512bits, fresh40-term actual-Xi jet over the
FULL inherited real-critical interval, calibrated lambda_(64), N32,
r/y=1/2, outerR=7/8 and inherited per-node512-bit complete256-cell cover.
Use exactly64equalclosedangular arcs and the SAME joint-polynomial
Horner/Cauchy calculation proved in JP. No tuning, new precision, point,
radius, Taylor degree, contour split, source window or alternative M bound.

Freshly evaluate one full-interval jet per point. Real symmetry and
f6(t)=0 permit exactly the same algebraic cancellation, not midpoint
substitution. Authenticate JP/QT/HA source and runtime before evaluating.
Do not rerun a favorable subset or substitute a failed source.

## Complete outcomes and evidence

Retain all26 positions and every failed prerequisite, arc comparison,
or parent-matching result. If a prerequisite blocks contour evaluation,
retain64explicit NOT_EVALUATED_DUE_GUARD slots and the exact reason.
A failed strict bound is UNRESOLVED, not root nonexistence.
Require the FULL HA root rectangle to lie inside the unknown-source
transport disc for all admissible t,y,r.

To keep the fixture bounded, preserve shared source/geometry fields once,
the fresh signed jet's canonical stream hash and real coefficient
enclosures, and all64 ordered arc decisions with exact error/margin
endpoints and a hash of the complete uncompressed arc records. Every
replay regenerates the entire signed jet and uncompressed arcs before
checking these summaries; a stream hash is not substituted for fresh
arithmetic. All unresolved outcomes remain in the primary denominator.

Arithmetic remains MIXED directed balls/exact rationals/complete finite
coverage. Caps:26points,40jet terms,32polynomial terms,64arcs,512bits,
24MBJSON,800000JSONnodes,depth24,container8000,4096-bitintegers.
Inherited source blobs have24MBcap. No arithmetic-domain extension occurs.

## Theorem and scope

Apply JP's general local Rouche theorem separately at each source point.
A complete strict circle plus full-rectangle containment identifies the
unique simple transported companion root with its already certified HA
root. The local theorem requires no innerness premise. Failed sufficient
guards assert neither root absence nor any impossibility result.

Report known-control and25-untried counts separately. This is a complete
fixed finite panel, not a cofinal theorem, growing-height assertion,
independent zero census, physical capture result, RH or novelty claim.
