...
|---|---|---|---|
| [generalized local coefficient powers](generalized/README.md) | exact recurrence, Laurent, finite-Fourier, and branch algebra | in the hyperbolic positive branch, complex coefficient powers are rational exactly at nonnegative integers; on an irrational tempered orbit, absolute powers are rational exactly at even nonnegative integers; rational rotations have an exact zero, branch, period, and integer collision-spectrum census | three narrow local Gate-0 verdicts; rational-orbit degrees are not uniform in the angle denominator; no global Euler product, automorphic nonintegral symmetric power, novelty, or zero claim |
