...
|---|---|---|---|
| [generalized local coefficient powers and parents](generalized/README.md) | exact recurrence, Laurent, finite-Fourier, branch, exponent-lattice, and symmetric-tensor algebra | four scalar chambers are classified; rational rotations have an exact branch/zero census; honest symmetric parents explain the integer shadows; a rank-\(n\) filtered spectrum drops from \(\binom{n+d}{n}\) to \(\binom{n+d}{n}-\binom{d}{n}\) on the determinant-one torus | integer rational-orbit degrees have uniform exponent-dependent bounds, while the fixed-noninteger uniform-degree question is separate; finite-state no-go results do not exclude infinite or categorical parents; no global Euler product, automorphic lift, novelty, or zero claim |
...
| all-`r` tensor/symmetric-power moment ladder | exact `SU(2)` weight extraction, Clebsch--Gordan recurrences, closed invariant counts through degree eight, and rank-aware generic classical-group comparison | independent `Std x Sym^r` and principal `Sym^(2r+1)` moments agree through degree four and split first at degree six by `3r(r+1)(r+2)/2`; the eighth gap is `2r(r+1)(r+2)(3r^2+6r+5)` | `USp` is polarization-matched to the product for even `r`; for odd `r>=3`, matched `SO(2r+2)` splits at degree four by `2r-1`, while `r=1` is exactly the full `SO(4)` trace law; compact Haar algebra proves no finite-family equidistribution, monodromy, novelty, or zero theorem |
| high-rank symmetric-power Haar boundary | oscillatory phase equidistribution, exact endpoint-mass expansion, regular variation, and a separate endpoint scaling for divergent moments | `chi_n` converges weakly to `W=sin(U)/sin(Theta)` with tail `16/(9pi^2)x^-3`; principal and product ladder limits are `W` and `ZW`, with distinct tail constants, while `B_k(n)` grows as `(n+1)^(2k-3)` for `k>=2` | the limits have variance one but no third absolute moment; raw moments are not uniformly integrable, and no growing-r arithmetic equidistribution, monodromy, automorphy, novelty, or zero theorem follows |
