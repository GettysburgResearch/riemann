...
|---|---|---|---|
| [generalized local coefficient powers and parents](generalized/README.md) | exact recurrence, Laurent, finite-Fourier, branch, exponent-lattice, and symmetric-tensor algebra | four scalar chambers are classified; rational rotations have an exact branch/zero census; honest symmetric parents explain the integer shadows; a rank-\(n\) filtered spectrum drops from \(\binom{n+d}{n}\) to \(\binom{n+d}{n}-\binom{d}{n}\) on the determinant-one torus | rational-orbit degrees are not uniform; finite-state no-go results do not exclude infinite or categorical parents; no global Euler product, automorphic lift, novelty, or zero claim |
