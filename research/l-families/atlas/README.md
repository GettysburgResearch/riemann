...
|---|---|---|---|
| [generalized local coefficient powers and parents](generalized/README.md) | exact recurrence, Laurent, finite-Fourier, branch, exponent-lattice, and symmetric-tensor algebra | four scalar chambers are classified; rational rotations have an exact branch/zero census and a sharp bounded/unbounded degree gate; honest symmetric parents explain the integer shadows; a rank-\(n\) filtered spectrum drops from \(\binom{n+d}{n}\) to \(\binom{n+d}{n}-\binom{d}{n}\) on the determinant-one torus | no quantitative noninteger degree-growth rate, higher-rank dense-orbit classification, global Euler product, automorphic lift, novelty, or zero claim; finite-state no-go results do not exclude infinite or categorical parents |
