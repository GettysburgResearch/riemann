...
|---|---|---|---|
| [generalized local coefficient powers](generalized/README.md) | exact recurrence, Laurent, finite-Fourier, branch, and finite-state algebra | hyperbolic positive powers are rational exactly at nonnegative integers; irrational tempered absolute powers exactly at even nonnegative integers; rational rotations have an exact branch/zero census; fixed-branch irrational complex powers exactly at nonnegative integers | four narrow local Gate-0 verdicts; rational-orbit degrees are not uniform; the finite-state no-go is not infinite-dimensional; no global Euler product, automorphic nonintegral symmetric power, novelty, or zero claim |
