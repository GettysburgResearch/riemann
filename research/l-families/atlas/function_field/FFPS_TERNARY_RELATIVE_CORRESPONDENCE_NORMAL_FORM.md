# Ternary relative source and Frobenius support-shift normal form
...
Let \(Q_1,Q_2\equiv1\pmod6\) be odd prime powers, and put
...
There is also a positive geometric support normal form. The surviving centered
incidence class
...
does not acquire an independent partial-Frobenius isomorphism. The formal
supports produced by successive one-coordinate Frobenius shifts are
...
Work in the faithful \(E\)-linear geometric support-function module
generated by the whole incidence plane \(J\) and the graph indicators
\(\Gamma_n\), where \(E\) has characteristic zero. The \(H_n\) are linearly
independent in this module. Hence there is no finite-dimensional closure
under the forward graph shift, but they form one free generator over its
polynomial ring:
...
support-symbol language even though the clean finite trace-kernel matrix has
full rank. This is a viable bookkeeping category for a successor, not yet a
closed-point Adams formula or a theorem in an arbitrary correspondence
quotient.
...
do not control the finite trace-kernel matrix rank after native coupling:
...
native physical phase variables restores full matrix rank on the clean
physical panel. This is not a Betti-rank or arbitrary categorical-rank
claim; those are different notions of rank.
...
For \(n>0\), \(\Gamma_n\ne\Delta\). Hence \(H_0\) is not equal to its
forward-shifted geometric support class. The projector \(\Pi_0\) cannot
repair this:
...
## 6. Exact Frobenius-graph support-shift module

Fix a characteristic-zero field \(E\). Let \(\mathscr S_E\) be the
\(E\)-linear geometric support-function module generated by the indicator of
the full incidence plane \(J\) and the extension-by-zero constant indicators
of the irreducible graphs \(\Gamma_n\), all after base change to an algebraic
closure. Equivalently, one may use the corresponding support-graded
constructible-function or sheaf-\(K_0\) classes, provided generic-support
maps remain faithful.

This is not a Chow-correspondence quotient and not the quotient by
trace functions on \(\mathbf F_Q\)-points: in the latter quotient all
\(\Gamma_n\) agree with the diagonal. The packet's independence statement
lives specifically in \(\mathscr S_E\).

The curves \(\Gamma_n\) are distinct irreducible geometric supports. For any
finite linear relation among \(H_0,\ldots,H_N\), evaluate generically off
every graph and then at the generic point of each \(\Gamma_n\). The resulting
triangular system forces every coefficient to vanish. Therefore
...
If \(T\) denotes the **forward graph shift** on these support classes, then
...
1. No finite-dimensional \(E\)-subspace of \(\mathscr S_E\) containing
   \(H_0\) is stable under every forward graph shift. A bounded finite list
   of these support symbols cannot restore the separable Adams hypothesis.
2. The whole tower has one generator over the shift semigroup. It is a
   compressed exact support-symbol representation, not a categorical
   partial-Frobenius isomorphism or a \(Q\)-rank Fourier decomposition.
...
This is the promised geometric support-shift normal form. It records only
the positive semigroup orbit; it does not assert that partial Frobenius is
an autoequivalence inside a new Adams-compatible category.
...
\[
 \boxed{
 \text{full clean trace-kernel matrix rank }M
 \quad\text{coexists with}\quad
 \text{one-generator, divisor-sparse support bookkeeping}.}
\tag{7.3}
\]

The next categorical target is a trace functor on this support-shift module which
...
3. **A finite graph-symbol list cannot close the forward support shift.**
   The geometric support orbit is linearly independent at every depth in
   \(\mathscr S_E\). This is not a statement in every correspondence or
   trace-function quotient.
...
- The support-shift module may admit a useful correspondence trace theory despite
...
| clean trace-kernel matrix ranks (0.4) | **PROVED BY POSITIVE-DEFINITE CONJUGATION** |
...
| no independent partial-Frobenius isomorphism supplied by relative subtraction | **PROVED FOR THE NATURAL SUPPORT CLASS; NO NEW CATEGORICAL ISOMORPHISM CONSTRUCTED** |
| graph-orbit independence (6.1) | **PROVED IN THE FAITHFUL GEOMETRIC SUPPORT MODULE \(\mathscr S_E\)** |
| one-generator forward shift modules (6.2)--(6.3) | **CONSTRUCTED EXACTLY IN \(\mathscr S_E\)** |
