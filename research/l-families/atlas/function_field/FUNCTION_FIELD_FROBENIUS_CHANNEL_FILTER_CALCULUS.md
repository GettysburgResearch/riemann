...
Choose three factorizations over a coefficient field (K) containing the
coefficients of (P_C) and (sqrt q):
...
If the filter is required over the original arithmetic coefficient field,
each chosen factor must be Galois-stable.  Define the finite coefficient
operator by multiplication of the generating function with
...
If (Q) is a proper Galois-stable factor of (P_C), (0.5) retains exactly
the reciprocal roots of (Q) in that channel.  Over a splitting field an
individual root can be selected.  Over the arithmetic base field, roots in
one Galois orbit cannot be separated by a base-field polynomial stencil.
...
- (1/P_C(XY/q)) is analytic because (r^2<q^{1/3}<\sqrt q);
- deleted axis factors first vanish at modulus (sqrt q);
- deleted mixed factors first vanish at (|XY|=q).

Hence the right side of (0.8) is analytic on every such bidisc.  Cauchy
gives
...
For (i=h+a,j=h+b), take any
(r<q^{1/6}) sufficiently close to (q^{1/6}).  Equation (2.1) becomes
(0.9).  No cancellation between Frobenius phases is used.
...
Nulling all three channels has channel-depth sum (6g) (three univariate
factors of degree (2g)).  Thus no
fixed-depth memberwise finite-difference filter can remove generic
Frobenius channels throughout a growing-genus family.  A family-uniform
construction must instead exploit a common factor, average/cohomological
cancellation, or a representation-valued observable.
...
coprime and all have degree (2g), the required axis depth is (2gN).
...
2. if genus grows, filter depth must grow unless the family has common
   numerator structure;
...
| Galois-stable channel criterion | **PROVED BY BASE-FIELD FACTORIZATION** |
...
| fixed-depth uniform filter for growing genus | **REFUTED GENERICALLY** |
