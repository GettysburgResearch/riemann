...
def conv_beta_g_limit(limit: int) -> int:
    ...
    for n in range(1, limit + 1):
        v = sum(beta(d) * g(n // d) for d in divisors(n))
        assert v == (1 if n == 1 else 0), (n, v)
        checks += 1
    return checks
...
def owner_identity_limit(limit: int) -> int:
    ...
    for n in range(2, limit + 1):
        ...
        for p, e in fs.items():
            cp = 2 if p == 67 else 1
            lhs = beta(n) * e
            rhs = -cp * sum(beta(n // (p ** a)) for a in range(1, e + 1))
            assert lhs == rhs, (n, p, lhs, rhs)
            checks += 1
    return checks


# Polynomial in t, represented low degree first.
...
def opmul(A: Dict[int, list[F]], B: Dict[int, list[F]]) -> Dict[int, list[F]]:
    ...
    for ma, pa in A.items():
        for mb, pb in B.items():
            assert ma & mb == 0  # each formal shift appears at most once here
            m = ma | mb
            out[m] = padd(out.get(m, [F(0)]), pmul(pa, pb))
    return out


def symmetric_homotopy_check(rs: list[F]) -> dict:
    k = len(rs)
    native = {mask: F((-1) ** mask.bit_count()) for mask in range(1 << k)}
    for mask in native:
        prod = F(1)
    ...
    for i, ri in enumerate(rs):
        ...
        for j, rj in enumerate(rs):
            ...
            prod_op = opmul(prod_op, Kj)

        # r_i (I-U_i) times the integrated product.
        for mask, poly in prod_op.items():
            c = ri * pint01(poly)
            rhs[mask] = rhs.get(mask, F(0)) + c
            rhs[mask | (1 << i)] = rhs.get(mask | (1 << i), F(0)) - c

    assert rhs == native, (rhs, native)
    return {
        "formal_primes": k,
        "monomials": len(native),
...
def tilt_energy_check(limit: int) -> dict:
    ...
    for n in range(2, limit + 1):
        ...
        integrated += float(ratio) / (2.0 * n * __import__("math").log(n))
    # Elementary comparison: sum_{n<=X} 1/(n log n) <= 2 + log log X.
    import math
    ...
    return {
        ...
        "max_beta2_over_g": str(max_ratio),
...
def smoothing_local_factor_check() -> dict:
    ...
    checks = 0
    for th in thetas:
        # (1-x)/(1-theta x) = 1 - (1-theta) sum_{k>=1} theta^(k-1) x^k.
        coeffs = [F(1)] + [-(1 - th) * (th ** (k - 1)) for k in range(1, 12)]
        # Verify multiplication by 1-theta x through degree 11.
        out = [F(0)] * 13
        ...
        checks += 11
    return {"theta_fixtures": len(thetas), "coefficient_checks": checks}


def five_three_numerator_check() -> dict:
    # Formal a=2^{-z}; 5P2+3P3 = -3(a-1)(a-2).
    lhs = {0: F(-6), 1: F(9), 2: F(-3)}
    rhs = {0: F(-6), 1: F(9), 2: F(-3)}
    assert lhs == rhs
