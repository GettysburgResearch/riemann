...
def exact_checks(limit: int = 5000) -> dict:
    ...
    assert 4 < 7                 # 1/sqrt(7) < 1/2
    assert 16 < 18               # sqrt(2) > 4/3
    assert 1331 < 1600           # (11/4)^3 < 25
    ...
    # 2,3 valuation tables for squarefree core d=5, mu(d)=-1
    r2_expected={
        (0,0):-1,(0,2):1,(1,0):3,(1,1):-2,(1,2):-1,(2,0):-2,(2,1):2
    }
    r3_expected={
        (0,0):-1,(0,1):6,(0,2):-5,(1,1):-5,(1,2):5,
    ...
    # y=2x-1 and 5y-x-1-3x^2 = -3(x-1)(x-2)
    # coefficients high to low are [-3,9,-6].
    common_poly=(-3,9,-6)
    assert common_poly == (-3, 9, -6)
    assert common_poly[0]*(1**2)+common_poly[1]*1+common_poly[2] == 0
    assert common_poly[0]*(2**2)+common_poly[1]*2+common_poly[2] == 0

    core={
        "fixed_product":{"j":3,"P":6,"n":24,"residual":str(residual)},
        "rough_store":{
            "P":30,"p":5,"u":2,"block":[2,10],
...
def main() -> None:
    ...
    result={
        ...
        "scope":{
            "fixed_product_frontier_chain_proved":False,
