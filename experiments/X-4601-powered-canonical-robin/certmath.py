...
class Dyadic:
    ...
    def mul(self, other: "Dyadic") -> "Dyadic":
        ...
        return Dyadic(self.bits, pmin // scale, -((-pmax) // scale))
    ...
    def contains_fraction(self, q: Fraction) -> bool:
        return (
            self.lo * q.denominator
            <= q.numerator * self.scale
            <= self.hi * q.denominator
        )
    ...
    def to_json(self, digits: int = 36) -> dict[str, object]:
        return {
            ...
            "lower_decimal_outward": dyadic_decimal(
                self.lo, self.bits, digits, upper=False
            ),
            "upper_decimal_outward": dyadic_decimal(
                self.hi, self.bits, digits, upper=True
            ),
        }
...
def dyadic_decimal(numerator: int, bits: int, digits: int, *, upper: bool) -> str:
    ...
    rounded = -((-scaled) // denominator) if upper else scaled // denominator
...
def atanh_log_unit(y: Fraction, *, bits: int, terms: int) -> Dyadic:
    ...
    z = (y - 1) / (y + 1)
    z_interval = Dyadic.fraction(z, bits)
    z_squared = z_interval.mul(z_interval)
    power = z_interval
    ...
    tail = power.div_pos_int(2 * terms + 1).div_positive(one.sub(z_squared))
...
def log_dyadic(x: Dyadic, *, terms: int) -> Dyadic:
    ...
    denominator = 1 << x.bits
    lo = log_fraction(Fraction(x.lo, denominator), bits=x.bits, terms=terms)
    hi = log_fraction(Fraction(x.hi, denominator), bits=x.bits, terms=terms)
...
def euler_gamma_interval(bits: int, log_terms: int, harmonic_cutoff: int) -> Dyadic:
    ...
    harmonic = harmonic_interval(harmonic_cutoff, bits=bits)
    log_n = log_fraction(
    ...
    correction = Dyadic(
        bits,
        Dyadic.fraction(
            Fraction(1, 2 * (harmonic_cutoff + 1)), bits
        ).lo,
        Dyadic.fraction(Fraction(1, 2 * harmonic_cutoff), bits).hi,
    )
    return harmonic.sub(log_n).sub(correction)
...
def robin_rhs(n: int, params: RobinParameters) -> Dyadic:
    ...
    log_n = log_fraction(Fraction(n), bits=params.bits, terms=params.log_terms)
    log_log_n = log_dyadic(log_n, terms=params.log_terms)
    return exp_gamma(params).mul(log_log_n)
