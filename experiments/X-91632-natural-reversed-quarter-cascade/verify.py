...
def coeff(ns):
    ...
    for j,n in enumerate(ns):
        ...
        be -= C[n].rat(w_num,w_den)
        ga -= B[n].rat(3*w_num,w_den)
        ga -= (C[n]*LOG4).rat(j*w_num,w_den)
        ga += D[n].rat(w_num,w_den)
    return al,be,ga
...
for k in range(BASE*LEFT,BASE*RIGHT+1):
    ...
    checks+=1
result={'classification':'PASS_NATURAL_REVERSED_QUARTER_CASCADE_K4_ENDPOINT_CERTIFICATE','digits':DIG,'atanh_terms':NTERMS,'checks':checks,'common_cells':BASE*(RIGHT-LEFT),'beta_nonnegative_cells':betapos,'maximum_upper_scaled_integer':str(maxhi),'scale_integer':str(S),'maximum_upper_decimal':f'{maxhi/S:.18f}','maximum_location':where,'runtime_seconds':time.time()-t0,'scope':'Exact integer/fixed-point interval arithmetic. Square roots use integer isqrt enclosures; logarithms use atanh partial sums with a positive rigorous tail. On each common cell beta<0, so the only critical point is a minimum and both one-sided endpoints suffice. Activated knot values are checked separately. Scalar sign only; no physical capacity theorem or RH.'}; Path(__file__).resolve().parent.joinpath('results','verification.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n'); print(result['classification']); print(json.dumps(result,indent=2,sort_keys=True))
