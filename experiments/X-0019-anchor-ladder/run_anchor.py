#!/usr/bin/env python3
"""
X-0019 -- Decide any dyadic positive-anchor cone: generalization of run_w4.
...
def main() -> int:
    ...
    ap.add_argument("--anchor", type=str, required=True, help="w as p/q")
    ap.add_argument("--x", type=str, required=True, help="x as p/q with x*x = w")
    ...
    for name, p in (("low", pl), ("high", ph)):
        x = p.get("x", {})
        if (x.get("numerator"), x.get("denominator")) != (X.numerator, X.denominator):
            raise SystemExit(f"{name} point is not x = {X}")
    ...
    tag = f"w{W.numerator}-{W.denominator}"
    candidate_path = HERE / "results" / f"{tag}-candidate.json"
    candidate_path.write_text(json.dumps({
        "schema": "riemann.x9312-positive-anchor-b0.v1",
        "anchor": {"numerator": W.numerator, "denominator": W.denominator},
        "b0_interval": {
    ...
    verdict_path = HERE / "results" / f"{tag}-verdict.json"
    import os
    ...
    if args.output is None:
        args.output = HERE / "results" / f"{tag}-run.json"
