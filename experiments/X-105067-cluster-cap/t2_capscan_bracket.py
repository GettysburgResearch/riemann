...
def count_zeros(a,b,reals,pairs,N=800001):
    ...
    return len(idx), list(zip(tv[idx], tv[idx+1])) if len(idx) else []
...
for trial in range(600):
    ...
    for (lo,hi) in zlocs:
        if not any(lo < x+y and hi > x-y for (x,y,mm) in deep):
            if not any(lo < x+y and hi > x-y for (x,y,mm) in other):
                viol.append(('CONF',trial,float(lo)))
import collections
