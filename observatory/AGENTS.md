...
3. Keep numerical providers pure and bounded. Validate through Spec; never evaluate arbitrary browser-submitted Python or JS. A trusted source extension is code review, not a runtime upload feature.
...
Suggested next deliverable: v0.2 in ROADMAP, not a framework rewrite as the only outcome. Separate numerical-contract work, view/state work, and storage work so contributors can test them independently.
