...
Created: 2026-08-14  
Depends on: `L-91680`; the exact component-entropy identity in `L-91363`; the published bound `psi(x) >= 0.9x` for `x >= 41` already frozen in the direct-Euler lineage; replay `X-91682`  
RH status: **unproved**
...
In particular `lambda_P(n)>=Lambda(n)` in the only cases where `Lambda(n)` is nonzero. The replay verifies this Boolean identity exactly through `n=4489`; the proof of (L-91682.8) is the elementary subset cancellation displayed above and is valid for every `n`.
...
and every term on the right is positive in the retained finite-Euler range. Thus adjoining primes can only decrease the positive forcing.
