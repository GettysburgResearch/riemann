...
4. **A cheap diagnostic.** Since $t^{*}=\min\{x^{\mathsf T}Qx:\eta^{\mathsf T}x=1\}$ and the minimiser is explicit, $t^{*}<0$ is *equivalent* to the existence of a normalised test vector with negative Weil energy, i.e. to a positivity violation in the even sector. So one need not compute an inertia at all to screen for violations: solve one linear system and check a sign. On the ill-conditioned Weil matrices this is far cheaper than a congruence, though it inherits the same precision requirement.
