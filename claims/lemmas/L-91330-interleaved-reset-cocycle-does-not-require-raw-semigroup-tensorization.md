# L-91330 — An interleaved positive reset cocycle does not require completed-state tensorization
...
Equation (L-91330.1) is the scalar compatibility check for one operation. Equation (L-91330.2), not a raw matrix product equality, is the load-bearing typed statement.
...
Apply (L-91330.2) to `u_0`. Then apply it to the child measure `mathfrak S(u_1)`, and continue. Positivity and linearity give, at every finite depth `k`,
...
For PR #399, the positive matrices `N_p` and the one-step identity are available. What is not yet available is a complete proof of (L-91330.2) in the actual endpoint/block/Schur coordinates, including:
...
The four-state route of `L-91327` is one possible construction of this missing typed reset. The one-prime Hall corridor of `L-91331` provides another concrete local input. Neither is completed here.
