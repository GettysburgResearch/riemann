...
\[
C(1+T)^{24}
 e^{96\theta T+C T^{3/4}(\log(2T))^2}.
\]

Since `96 theta<delta^2/2`, the lower exponential rate is strictly larger.
Letting `T` tend to infinity gives a contradiction.  Therefore zeta has no zero
with real part greater than `1/2`.  The functional equation gives the symmetric
half, and all nontrivial zeros lie on the critical line.
