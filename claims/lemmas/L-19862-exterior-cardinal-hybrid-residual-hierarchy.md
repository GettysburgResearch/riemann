...
## 1. Finite space and exact Xi target
...
For large `L`, `v_(L,N)` is nonzero. Normalize it in the ordinary finite-space
metric and denote the unit vector by `p_(L,N)`.

Its exact global residual is

\[
 W_*=J_{\Xi}-\iota_Lv_{L,N},
 \tag{L-19862.3}
\]
...
up to the harmless convention factor between full and half interval length.
...
with sufficiently large fixed `c_B` gives
...
every large unit `L`-block on which (L-19862.13) holds simultaneously and the
...
Using (L-19862.14) linearly, choose exterior arithmetic images `J_(w_j)` with
...
\[
 \mathcal U_{L,N}
 =\operatorname{span}\{J_{\Xi},J_{w_1},\ldots,J_{w_{d-1}}\}
 \tag{L-19862.18}
\]
...
After the harmless target normalization, `S_(L,N)` is an isomorphism onto the
complete finite CCM space. Thus exact finite surjectivity and the target source
now occur in one reservoir.
...
This completely resolves the common-reservoir spectral objection raised in the
revised review. The gap is constant rather than `d_6`.
...
```text
one exact Xi lift            gives exponentially small residual energy;
...
- The remaining RH-bearing input is now precisely an affine one-sided estimate
