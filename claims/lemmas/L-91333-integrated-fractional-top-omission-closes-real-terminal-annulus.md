# L-91333 — An integrated fractional-column top omission closes the real terminal annulus
...
Hence every fractional correction in (L-91333.1) is nonnegative whenever both adjacent nodes are active. The partially active final breakpoint is also nonnegative.
...
- If only the first breakpoint is full, then either `s<2q` or the second is only partially active. In both cases
  ...
  \]
  so (L-91333.4) contributes more than `1/2`.
