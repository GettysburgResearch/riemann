# L-90307 — Aggregate inertia eliminates the unknown current from the determinant defect
...
Created: 2026-08-10  
Dependencies: PR #357 `L-90301/L-90303`; finite-dimensional Cauchy–Schwarz  
...
For the relative Q4 jet of PR #357 `L-90303`, after one common source-centering shear,
...
Substitution of (L-90307.5) proves (L-90307.6).
...
is optimized out by Cauchy–Schwarz before any rowwise absolute value. Large current energy `B` strengthens the positive determinant term `AB`; it never worsens the final bound.
...
A row-dependent shear cannot be inserted before aggregation without changing the physical synthesis operator.

## 6. Sharpness and mutation firewall

The factor `A/(A-F)` cannot in general be deleted. The exact verifier `X-90303` contains a rational two-row witness with
...
2. the source-only negative-mass bound;
