...
Claim ID: `L-105061`
Status: **INTERFACE — sub-lemmas [I-PF] and [THRESHOLD] PROVED here; the COUNT CAP is now PROVED on the separated/subcritical regimes (`L-105063`, deposited 2026-08-23; A′ = 6 on C-0∪C-1∪C-2, 2k on the depth-k shallow ladder) and remains OPEN only on the residual regimes (R-C1) deep clusters and (R-C2) supercritical shallow mass (`L-105063` §4) — RH NOT ADDRESSED**
Created: 2026-08-22
...
in the regimes C-0 (no overhang), C-1 (isolated site), and C-2 (deep pairs
with disjoint `R4`-fattened intervals + subcritical shallow mass), and with
the depth-dependent constant `2k` on the all-shallow ladder C-3 (`k`
unbounded; regime-wise `A' = max(6, 2k)`). The pinned statement remains open ONLY on `L-105063`
