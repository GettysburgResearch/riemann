# L-3903 — Matched-pole barycentric Pick annihilators
...
Add a `matched-pole-real-pick` producer mode to the active Arb branch PR #56. The existing `real-pick-rayleigh` exact checker already accepts the resulting rational vector, so the proof path requires no new trusted matrix code:
