# L-15102 — Radical compression and exact localization-leakage residual
...
## 1. Abstract radical compression identity
...
Then, for every local vector `w in D cap ran(P)`,
...
Assume the compression of `q` to

\[
 K_P=\operatorname{ran}(P)
\]

is a closed lower-bounded form represented by a self-adjoint operator `A_P`.
Suppose `p` belongs to the operator domain and put
...
Then, for every local `w` with `w perpendicular p`,

\[
 \boxed{
 \langle(A_P-\mu_pI)p,w\rangle
 =q(p,w)
 =-q(t,w).}
 \tag{L-15102.6}
\]
...
Let `k=E(h)` be the exact radical target of `L-15101`, let `P_lambda` be a
smooth inversion-even localization, and put

\[
 p_\lambda=P_\lambda k,
 \qquad
 t_\lambda=(I-P_\lambda)k.
\]

Whenever the semilocal Weil form is the exact compression of the global form on
