# L-14315 — Weighted projective radical-tail duality and exact Poisson support transport
...
Consequently the numerator in (L-14315.5) is generated exactly by the ordinary
...
This proves (L-14315.5). Equation (L-14315.6) is the elementary expansion of
...
which is (L-14315.10). QED.

## Exact finite coordinate form

If `H` is finite dimensional, the left side of (L-14315.5) has the closed
Schur expression
...
`X-14308` checks (L-14315.12) against the radical-tail dual on an exact
rational model and obtains `4/11` by both routes.

## What remains analytic

Equation (L-14315.10) is exact, but it does not say that the Weil cross-form
...
ordinary leakage estimate in `L-14314` is an input to, not a substitute for,
this theorem.
...
- The Poisson identity requires the exact source constraints from `L-14314`.
- No boundedness estimate of the form (L-14315.13) is proved here.
...
1. Use a symmetric rational form with a declared radical and independently
   verify both identities in (L-14315.6).
...
Use Suzuki's explicit localized form to bound the operator in (L-14315.13)
