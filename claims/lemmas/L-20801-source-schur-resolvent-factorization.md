# L-20801 — Source-canonical shorting is one scalar Weyl resolvent
...
be a nonzero source functional and put
...
Since \(A|_W\succ0\), the restricted quadratic is strictly convex. Its unique
...
For \(\operatorname{Im}z>0\), \(m_A(z)\) is a scalar Herglotz resolvent. Its
poles are precisely those full eigenvalues visible to \(\ell\), and the
corresponding zeros of \(S_A\) are the full eigenvalues not already carried by
the constrained block. This is the finite source-canonical Weyl/Krein reduction.
...
When the quotient is negative, (L-20801.9) shows that the full finite operator
...
- neither operation changes the sign in (L-20801.15);
- proving the required cofinal sign is a genuine arithmetic Weyl-function
  inequality, not another conditioning lemma.

The independent exact consumer is `X-20802`. No RH conclusion is claimed in
this lemma.
