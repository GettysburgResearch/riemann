# M-25801 — PADT production and adversarial review protocol

Claim ID: `M-25801`  
Title: Production order, certificate schema, and automatic rejection tests for Prime-Anchored Digit Transport  
...
Produce or refute `PADT(K)` on the actual top reflected source. The producer may
use numerical optimization to nominate a flow, but every accepted certificate
...
PR #236  fixed-ratio and parity-comb mutations
PR #254  signed carry dipole and cluster descent
```

A certificate is invalid if one of these mutable dependencies is referenced
without a frozen commit.

## 3. Production ladder

### Stage A — exact algebra controls
...
The flow solver is a discovery aid only.

### Stage D — `K=8` adversarial replay

Search specifically for:

- a source fiber with no adjacent sibling;
...
1. source-additive incidence for all labeled residual slots;
...
A production directory contains:
...
Every source tuple and flow edge has a stable ID. Repeated primitive evaluations
share one feature ID so correlated uncertainty is not widened independently.

## 5. Exact checker gates
...
- coefficientwise reconstruction;
- exact Möbius, binomial, and reflected signs;
- every cutoff and transition surface.
...
- internal edges occur twice with opposite orientation;
- both endpoint source projections agree outside the moved coordinate;
- hypercube siblings are present;
- no boundary is called internal.
...
- independent frequencies `t,s`;
- exact block kernel;
- complete cross terms;
...
## 6. Automatic mutation tests

Every checker must reject:
...
The higher-uniformity Type-II results of Matomäki--Radziwill--Shao--Tao--
Teräväinen provide a serious methodological comparison: contagion and scaling
can propagate approximate structure, but the published theorem is on almost all
short intervals. A `PADT` proof must be deterministic on every sufficiently
large block or prove a separate exceptional-set exclusion for the exact Mertens
mode.
...
A reviewer may promote `PADT(K)` only after:
...
```text
exact depletion algebra       proposed complete
finite flow schema             proposed complete
production PADT flow           open
PADT family                    open
RH                             unproved
```
