...
Let `a=sigma-delta`. Keeping one fixed small ordinate neighborhood of `gamma`, the principal term gives
...
The analytic remainder contributes at most logarithmic order to the cross term and bounded order to its square. Consequently, as `sigma->delta` through zero-free vertical lines,

\[
 \boxed{
 I_H(\sigma)
 ={\pi m^2|\widehat H(z_\rho)|^2\over|\sigma-\delta|}
 +O_\rho\!\left(\log{1\over|\sigma-\delta|}\right)
 +I_{\rm other}(\sigma),
 }
 \tag{T-22302.9}
\]

where `I_other` contains the remaining global zero/background contribution. If finitely many zeros share the same real part in a bounded ordinate range, their leading coefficients add.

Equation (T-22302.9) is a vertical tomography law: every off-critical zero line creates a positive inverse-distance spike whose leading coefficient cannot be canceled by another analytic term.
...
Conversely, under RH the supremum is finite on every such compact interval by the fixed-distance version of (T-22302.10). Thus
...
- the inverse-distance blow-up coefficient;
- finiteness under RH;
