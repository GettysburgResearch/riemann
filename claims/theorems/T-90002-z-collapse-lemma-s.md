...
## Addendum (post-review, same session)

The \(N>400\) envelope step uses that \(h(N)=2\sqrt2/\sqrt{N-1}-2/\sqrt N\) is decreasing in \(N\); this is true and elementary but was unstated in the first commit — recorded per the adversarial review. The review independently re-verified the two-scale identity at 50 digits (both parities, including \(q\mid Y\)), the growth-cancellation identity, all Theorem P margins, and the Corollary C window arithmetic; grades: Theorems P and Q VERIFIED (complete), Theorem S and Corollary C VERIFIED modulo the two named in-repo imports (T-90001 §1 floor bound; O-90004 Prop A). Consumers should also see L-90003 §5: for general ratios \(c\in[0.44,0.47]\) the profile has three sign changes — Theorem S's single crossing is a property of the realized range \(c\in[0.49,0.5)\), not of all \(c\).
