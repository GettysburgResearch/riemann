...
**Lemma S (PROMOTED: proved as `T-90002` Theorem S for \(X\ge10^5\) with explicit ±52 window; the collapse below is now unconditional with the sharper error \(O^*(5.7\cdot10^4X^{-3/2}\log X)\) — see T-90002 Corollary C. Original record follows; verified exactly to \(X=5\cdot10^5\).)** \(s_X(q)>0\) for all integers \(2\le q<c^*X\) and \(\le0\) for \(c^*X<q\le Y\), \(c^*=0.140852035013839925\ldots (digits refined in T-90002; the earlier 0.1408512 was a preliminary estimate)\) (root of an explicit \(N=7/N'=3\) cell equation), with \(O(1)\) ambiguous integers at the threshold. At \(X=5\cdot10^5\): zero exceptions among 64,497 integers below threshold (min \(\sqrt q\,s=+0.0256\)) and none above (max \(\sqrt q\,s=-0.0028\)).
